# -*- coding: utf-8 -*-
"""
S&P 100 Dashboard (SimFin + SQLite)
...
tab1, tab2, tab3, tab4, tab5 = st.tabs([
    "📈 Chart", "📊 Fundamentals", "💎 Bewertung", "🧮 DCF", "🗄️ DB",
])
...
with tab4:
    ...
    if inputs_a is None:
        ...
        g3 = (g2 + term_g) / 2 + 0.015
        g4 = (g3 + term_g) / 2
        g5 = (g4 + term_g) / 2
        growth_rates = [g1, g2, g3, g4, g5] + [term_g + 0.005] * max(0, n_years - 5)
        ...
        st.divider()
        st.subheader(f"🔬 Sensitivität · {company}")
        st.caption("Fair Value bei verschiedenen WACC/Terminal-Growth")
        ...
        for i, w in enumerate(waccs):
            for j, t in enumerate(tgs):
                sens[i, j] = run_dcf(inputs_a["rev_current"], growth_rates,
                                     fcf_margin, w, t, n_years,
                                     inputs_a["net_debt"], inputs_a["shares"],
                                     inputs_a["current_price"])["fv"]
...
# ============================================================
st.divider()
st.caption("Datenquelle: SimFin · Lokale SQLite-DB · Keine Anlageberatung")
